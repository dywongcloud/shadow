...
async fn firecracker_boots_real_microvms_isolated_per_tenant() {
    ...
    let a = be.provision(&spec("alpha")).await.expect("provision alpha microVM");
    let b = be.provision(&spec("beta")).await.expect("provision beta microVM");
    ...
    assert!(a.root.starts_with(run_dir.join("alpha")), "alpha run dir not under its tenant: {:?}", a.root);
    assert!(b.root.starts_with(run_dir.join("beta")), "beta run dir not under its tenant: {:?}", b.root);
    assert_ne!(a.root.parent(), b.root.parent(), "tenants must not share a run-dir parent");
    assert!(a.root.join("api.sock").exists(), "firecracker API socket missing (VM not started)");
