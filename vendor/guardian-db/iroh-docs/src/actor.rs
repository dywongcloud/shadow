...
enum Action {
    ...
    Shutdown {
        #[debug("reply")]
        reply: Option<oneshot::Sender<Result<Store>>>,
    },
    #[cfg(test)]
...
enum ReplicaAction {
    ...
    InsertLocal {
        ...
        hash: Hash,
        len: u64,
        #[debug("gc mutation guard")]
        _gc_guard: Option<tokio::sync::OwnedRwLockReadGuard<()>>,
        #[debug("reply")]
        reply: oneshot::Sender<Result<()>>,
    },
    DeletePrefix {
...
impl SyncHandle {
    ...
    pub(crate) async fn mutation_guard(&self) -> Option<tokio::sync::OwnedRwLockReadGuard<()>> {
    ...
    pub async fn insert_local(
        ...
        let gc_guard = self.mutation_guard().await;
        self.insert_local_guarded(namespace, author, key, hash, len, gc_guard)
            .await
    }

    pub(crate) async fn insert_local_guarded(
        &self,
        namespace: NamespaceId,
        author: AuthorId,
        key: Bytes,
    ...
    pub async fn delete_prefix(
        ...
        let (reply, rx) = oneshot::channel();
        let action = ReplicaAction::DeletePrefix {
            author,
            key,
            _gc_guard: match &self.mutation_gate {
                Some(gate) => Some(gate.clone().read_owned().await),
                None => None,
            },
            reply,
...
impl Actor {
    ...
    async fn run_async(mut self) {
        ...
        match reply {
            Some(reply) => {
                reply.send(flush_result.map(|()| self.store)).ok();
            }
            None => {
                if let Err(cause) = flush_result {
                    warn!(?cause, "failed to flush store during implicit shutdown");
                }
            }
        }
    }
