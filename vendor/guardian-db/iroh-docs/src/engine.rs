...
use crate::{
    Author, AuthorId, ContentStatus, ContentStatusCallback, Entry, NamespaceId, actor::SyncHandle,
    metrics::Metrics,
};
...
impl Engine {
    ...
    pub async fn spawn(
        ...
        let actor = match LiveActor::new(
            ...
            Ok(actor) => actor,
            Err(error) => {
                // LiveActor construction happens after the redb actor starts. If
                // endpoint/address-lookup setup fails, explicitly flush and stop
                // that actor before returning so its database lock is released.
                return match sync.shutdown().await {
                    Ok(_store) => Err(error),
                    Err(cleanup_error) => Err(anyhow::anyhow!(
                        "Docs Engine initialization failed: {error:#}; redb actor cleanup also failed: {cleanup_error:#}"
                    )),
                };
            }
        };
        ...
        let default_author = match DefaultAuthor::load(default_author_storage, &sync).await {
            Ok(author) => author,
            Err(error) => {
                // The live actor owns the sync/redb actor. Drive its normal
                // shutdown path and join it rather than dropping a raw JoinHandle,
                // which would detach the actor and retain database locks after a
                // partially initialized Docs::spawn.
                let mut cleanup_errors = Vec::new();
                let (reply, reply_rx) = oneshot::channel();
                match live_actor_tx.send(ToLiveActor::Shutdown { reply }).await {
                    Ok(()) => match reply_rx.await {
                        Ok(Ok(())) => {}
                        Ok(Err(cleanup_error)) => cleanup_errors.push(format!(
                            "live/redb actor shutdown failed: {cleanup_error:#}"
                        )),
                        Err(reply_error) => cleanup_errors.push(format!(
                            "live/redb actor dropped its shutdown reply: {reply_error}"
                        )),
                    },
                    Err(send_error) => cleanup_errors.push(format!(
                        "live actor rejected its shutdown request: {send_error}"
                    )),
                }
                if let Err(join_error) = actor_handle.await {
                    cleanup_errors.push(format!("live actor join failed: {join_error}"));
                }
                return if cleanup_errors.is_empty() {
                    Err(error)
                } else {
                    Err(anyhow::anyhow!(
                        "Docs Engine initialization failed: {error:#}; cleanup also failed: {}",
                        cleanup_errors.join("; ")
                    ))
                };
            }
