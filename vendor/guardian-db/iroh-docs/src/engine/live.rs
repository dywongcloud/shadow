...
impl LiveActor {
        ...
        match self.run_inner().await {
            ...
            Err(run_error) => {
                ...
                match shutdown_result {
                    Ok(()) => Err(run_error),
                    Err(shutdown_error) => Err(run_error.context(format!(
                        "live actor shutdown also failed: {shutdown_error:#}"
                    ))),
                }
            }
        }
    ...
    async fn run_inner(&mut self) -> Result<oneshot::Sender<anyhow::Result<()>>> {
        let mut i = 0;
        loop {
            i += 1;
            trace!(?i, "tick wait");
    ...
    async fn shutdown(&mut self) -> anyhow::Result<()> {
        ...
        let (gossip_shutdown_res, store_shutdown_res) = tokio::join!(
            // quit the gossip topics and task loops.
            self.gossip.shutdown(),
            // shutdown sync thread and commit its final redb write transaction.
            self.sync.shutdown()
        );
        gossip_shutdown_res?;
        store_shutdown_res?;
