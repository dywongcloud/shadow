...
use crate::{Author, DocTicket, NamespaceSecret, SignedEntry, engine::Engine};
...
impl RpcActor {
    ...
    pub(super) async fn doc_set(&self, req: SetRequest) -> RpcResult<SetResponse> {
        ...
        let len = value.len();
        let gc_guard = self.sync.mutation_guard().await;
        let tag = blobs_store
            .add_bytes(value)
            .temp_tag()
            .await
    ...
    pub(super) async fn doc_set_hash(&self, req: SetHashRequest) -> RpcResult<SetHashResponse> {
        ...
        let gc_guard = self.sync.mutation_guard().await;
        self.sync
            .insert_local_guarded(doc_id, author_id, key.clone(), hash, size, gc_guard)
            .await
            .map_err(|e| RpcError::new(&*e))?;
        Ok(SetHashResponse {})
    }
