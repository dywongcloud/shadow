...
enum ImportInner {
    ...
    #[debug("Entry")]
    Entry(n0_future::boxed::BoxFuture<Result<(ImportFileOutcome, iroh_blobs::api::TempTag)>>),
    #[debug("EmitDone")]
    EmitDone(Option<ImportFileOutcome>),
    Done,
}
        ...
        loop {
            match this.0 {
                        ...
                        Some(item) => match item {
                            AddProgressItem::Size(size) => {
                                context
                                    ...
                                    .size = Some(size);
                                return Poll::Ready(Some(ImportFileProgressItem::Blobs(
                                    AddProgressItem::Size(size),
                                )));
                            }
                            AddProgressItem::Error(err) => {
                                this.0 = ImportInner::Done;
                                return Poll::Ready(Some(ImportFileProgressItem::Error(
                                    err.into(),
                                )));
                            }
                            AddProgressItem::Done(tag) => {
                                ...
                                let handoff = tokio::spawn(async move {
                                    ...
                                    Ok::<_, anyhow::Error>((
                                        ImportFileOutcome { hash, size, key },
                                        tag,
                                    ))
                                });
                                this.0 = ImportInner::Entry(Box::pin(async move {
                                    handoff.await.map_err(|error| {
                                        anyhow::anyhow!(
                                            "import-file metadata handoff task failed: {error}"
                                        )
                                    })?
                                }));
                                // Dropping ImportFileProgress detaches `handoff`; that task owns
                                // the TempTag until set_hash returns after the actor's durable
                                // insertion, so request cancellation cannot expose an unrooted
                                // blob between RPC submission and actor guard acquisition.
                                continue;
                            }
                            item => {
                                return Poll::Ready(Some(ImportFileProgressItem::Blobs(item)));
                            }
                        },
                        None => todo!(),
                    }
                ...
                ImportInner::Entry(ref mut fut) => {
                    ...
                    match res {
                        Ok((outcome, tag)) => {
                            this.0 = ImportInner::EmitDone(Some(outcome));
                            return Poll::Ready(Some(ImportFileProgressItem::Blobs(
                                AddProgressItem::Done(tag),
                            )));
                        }
                        Err(err) => {
                            this.0 = ImportInner::Done;
                            return Poll::Ready(Some(ImportFileProgressItem::Error(err)));
                        }
                    }
                }
                ImportInner::EmitDone(ref mut outcome) => {
                    let outcome = outcome
                        .take()
                        .expect("ImportFileProgress outcome may be emitted only once");
                    this.0 = ImportInner::Done;
                    return Poll::Ready(Some(ImportFileProgressItem::Done(outcome)));
                }
                ImportInner::Done => return Poll::Ready(None),
            }
        }
    }
}
