...
impl BlobsProtocol {
    pub fn new(store: &Store, events: Option<EventSender>) -> Self {
        Self::with_store_lifecycle(store, events, true)
    }

    /// Create a protocol handler whose owner shuts the store down explicitly.
    ///
    /// Router shutdown still stops blob ingress and awaits this handler, but it
    /// leaves the actor alive so the owner can call `Store::sync_db` followed by
    /// `Store::shutdown` and propagate either failure.
    pub fn new_managed(store: &Store, events: Option<EventSender>) -> Self {
        Self::with_store_lifecycle(store, events, false)
    }

    fn with_store_lifecycle(
        store: &Store,
