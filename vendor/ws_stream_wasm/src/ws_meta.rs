...
impl WsMeta
{
		...
		-> Result< (Self, WsStream), WsErr >
	{
		...
		// A relay can send its first frame with the upgrade response. Firefox
		// determines MessageEvent.data from binaryType when that frame arrives,
		// not when the event listener later consumes it. Set this before awaiting
		// Open so the first frame cannot become an unsupported Blob.
		...
		// Transfer callback ownership without another await between disabling the
		// cancellation guard and constructing the stream. Installing onmessage
		// before waiting for Open preserves a frame sent with the upgrade response.
