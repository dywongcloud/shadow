            ...
            None => {
                ...
                match ret {
                    ...
                    | Some(Err(PathError::MaxPathIdReached)) => {
                        ...
                        // VENDORED FIX (fluid-hive, 2026-08-08): bound this queue.
                        //
                        // Both failure modes here are properties of the CONNECTION (its
                        // remote-CID budget or its max path-id), not of a moment in
                        // time — retrying in 333ms does not free either one. On a
                        // connection that stays exhausted, EVERY holepunch/addr-update
                        // trigger re-attempts open_path_on_conn for every pending addr
                        // against every connection, and each renewed failure pushed
                        // another COPY of the same `open_addr` here with no dedup and
                        // no cap — an unbounded, monotonically growing VecDeque.
                        //
                        // Measured in production under jemalloc profiling: 71,680 MiB
                        // (99.8% of live heap) in ONE stack, `RawVec::finish_grow ->
                        // VecDeque::grow -> RemoteState::open_path_on_conn ->
                        // RemoteStateActor::open_path_on_all_conns`, on a mesh node
                        // fielding frequent peer churn. This is that leak, fixed at its
                        // source rather than by filtering which addresses may reach
                        // `connect()` (a fleet-side mitigation attempted and reverted
                        // after it partitioned the mesh) — filtering dial CANDIDATES is
                        // not equivalent to bounding a RETRY QUEUE; only the latter
                        // cannot affect which peers are reachable.
                        //
                        // Dedup: an addr already queued needs no second entry — the
                        // existing entry is retried on the same 333ms tick. Cap: once
                        // queued past MAX_PENDING_OPEN_PATHS (comfortably above any real
                        // multi-path scenario), further duplicates for a connection that
                        // keeps failing are dropped rather than grown forever — this
                        // queue is a retry optimization, not a correctness guarantee, so
                        // a dropped duplicate costs nothing beyond waiting for the next
                        // trigger instead of this one.
                        const MAX_PENDING_OPEN_PATHS: usize = 256;
                        if self.pending_open_paths.contains(open_addr) {
                            trace!(?open_addr, "open_path already queued; not duplicating");
                        } else if self.pending_open_paths.len() >= MAX_PENDING_OPEN_PATHS {
                            warn!(
                                ?open_addr,
                                len = self.pending_open_paths.len(),
                                "pending_open_paths at cap; dropping duplicate retry \
                                 (connection's path/CID budget appears persistently exhausted)"
                            );
                        } else {
