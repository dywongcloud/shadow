...
import { Search, Database, RefreshCw, ChevronRight, Code2, Plus, Trash2, Save, Pencil, Table2, Play } from "lucide-react";
...
// of the document-collection JSON view above. Read-only: no edit/create/delete
...
export default function DataBrowserPage() {
  ...
  // ---- Tables (PostgreSQL) view state — entirely separate from the Documents
  // state above; switching view never mutates or reads the other's state.
  ...
  // Load the table list once, the first time the Tables view is opened.
  useEffect(() => { if (view === "tables" && sqlTables === null) loadSqlTables(); }, [view, sqlTables]);
  ...
  useEffect(() => { if (view === "tables" && activeTable && !sqlResult && !sqlLoading) runSqlQuery(); }, [view, activeTable]); // eslint-disable-line react-hooks/exhaustive-deps
