    ...
    *cloud.peers.write() = args.peers.clone();
    ...
    let public = if cloud.ingress != "ngrok" {
        if !auth::enforced() {
            ...
            host_switch_router(cloud.clone(), api_host, dash_hosts, dash_upstream, cloud.http.clone(), admin_router.clone(), public)
        }
    } else {
        public
    };
    ...
    let handler = move |req: Request<Body>| {
        ...
        async move {
            ...
            let host = req
                ...
                .to_ascii_lowercase();
            if host == api_host {
                return admin_ingress(cloud, admin, api_host, req).await;
            }
