...
export async function apiGet<T>(path: string, opts?: { fresh?: boolean }): Promise<T> {
  ...
  if (!opts?.fresh && hit && Date.now() - hit.at < GET_TTL_MS) return hit.data as T;
  const req = (async () => {
...
export function usePoll<T>(path: string, intervalMs = 3000, active = true) {
  ...
  useEffect(() => {
    ...
    // The recurring tick is always `fresh` — polling IS the caller's explicit
    // freshness cadence, so it must never be silently masked by a moments-old
    // cache entry (that would double the effective latency of a fast poller like
    // a live log tail).
    const id = active ? setInterval(() => load(true), intervalMs) : null;
