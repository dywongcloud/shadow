...
impl CloudState {
    ...
    /// host (local/direct/default deployment) is allowed; the apps domain
    /// (`*.{HIVE_APPS_DOMAIN}`, one label + apex) is always allowed; any other
    /// multi-label host must end with a configured deploy suffix (the ngrok
    /// zones — kept while `HIVE_INGRESS != dns`), else it's a foreign root.
    pub fn host_allowed(&self, host: &str) -> bool {
        host_matches_apps_domain(host, &self.apps_domain)
